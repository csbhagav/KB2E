...
public:
    ...
    void add(int x, int y, int z, bool flag) {
        /*
         * flag is set to true when data point is a test instance, false for train / validation instance
         */
        ...
        ok[make_pair(x, z)][y] = 1; // All instances ??
    }
    ...
    void run() {
        ...
        // fb_l.size() is the same as the test set size == fb_r.size() == fb_h.size()

        for (int testid = 0; testid < fb_l.size(); testid += 1) {
            int h = fb_h[testid]; // Subject of a relation
            ...
            for (int i = a.size() - 1; i >= 0; i--) {
                ...
                if (ok[make_pair(a[i].first, rel)].count(l) == 0)
                    filter += 1;
                if (a[i].first == h) // Correct prediction
                {
                    lsum += a.size() - i; // Rank of correct prediction
        ...
        cout << "left:"
             << lsum / fb_l.size() << '\t'
             << lp_n / fb_l.size() << "\t"
             << lsum_filter / fb_l.size() << '\t'
             << lp_n_filter / fb_l.size() << endl;
        cout << "right:"
             << rsum / fb_r.size() << '\t'
             << rp_n / fb_r.size() << '\t'
